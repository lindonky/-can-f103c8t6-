...
/***************************************************************************************
  ...
  *
  * 移植说明：本文件已由标准库移植至 STM32 HAL 库
  *           移植者只需修改下方【引脚配置区】的宏定义即可适配不同引脚
  *
  ***************************************************************************************
  */

/*【HAL库移植：头文件引用】*/
#include "main.h"			//HAL库总头文件，内含 stm32g4xx_hal.h
#include "OLED.h"
#include <string.h>
#include <math.h>
#include <stdio.h>
...
/*【HAL库移植：微秒级延时】*********************/
/**
  * 说明：STM32G4主频170MHz，远高于标准库版本的72MHz
  *       若不在I2C引脚翻转时加微量延时，通信速率可能超出SSD1306的承受范围（400kHz）
  *       此处使用DWT（数据观察点与跟踪）周期计数器实现精确微秒延时
  *       DWT延时纯寄存器轮询，不依赖中断，在RTos任务和初始化阶段均可安全使用
  */
#define OLED_DWT_us(n)     OLED_Delay_us(n)
static void OLED_Delay_us(uint32_t us)
{
	/*若DWT未初始化，则自动使能DWT周期计数器*/
	...
	/*SystemCoreClock在system_stm32g4xx.c中定义，G431配置为170000000*/
...
void OLED_W_SCL(uint8_t BitValue)
{
	...
	HAL_GPIO_WritePin(OLED_SCL_PORT, OLED_SCL_PIN, BitValue ? GPIO_PIN_SET : GPIO_PIN_RESET);

	/*【HAL库移植】添加延时，避免170MHz主频下I2C速率过快超出SSD1306承受范围*/
	OLED_DWT_us(1);
}
...
void OLED_W_SDA(uint8_t BitValue)
{
	...
	HAL_GPIO_WritePin(OLED_SDA_PORT, OLED_SDA_PIN, BitValue ? GPIO_PIN_SET : GPIO_PIN_RESET);
...
{
	/*【HAL库移植】在初始化前，加入适量延时，待OLED供电稳定*/
	/*此处用DWT延时替代标准库的空循环延时，更精确*/
	OLED_Delay_us(100000);		//延时100ms

	/*【HAL库移植】将SCL和SDA引脚初始化为开漏输出模式*/
	/*开漏模式下需外部上拉电阻，OLED模块通常板载4.7K上拉，无需另接*/
	GPIO_InitTypeDef GPIO_InitStruct = {0};
	GPIO_InitStruct.Pin = OLED_SCL_PIN | OLED_SDA_PIN;
	GPIO_InitStruct.Mode = GPIO_MODE_OUTPUT_OD;			/*开漏输出*/
	GPIO_InitStruct.Pull = GPIO_NOPULL;					/*依靠模块外部上拉*/
